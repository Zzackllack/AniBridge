...
- [x] Inspect current workspace changes and local agent instructions.
- [x] Verify remote main and the latest upstream release. The research base is six lock-only commits behind the recorded remote main.
- [x] Refresh the implementation base without discarding the specification or unrelated user changes.
- [x] Reproduce original-pin full tests in an isolated environment with correct repository metadata/default-path behavior.
- [x] Record Python, OS/architecture, uv version, base SHA, package version, and test output.
- [x] Keep an original-lock environment for differential checks.
...
- [x] Change the pin to the assessed released target (5.0.6 unless the specification is refreshed).
- [x] Regenerate the lock with `--upgrade-package aniworld`; inspect the complete diff.
- [x] Preserve remote main's independent Mako/AnyIO/SoupSieve updates or their current successors.
- [x] Run locked/frozen installation and `uv pip check`.
- [x] Re-run all existing tests before compatibility code changes.
- [x] Add subprocess real-package symbol/model/language contracts.
...
- [x] Introduce/use the local episode protocol at the current facade boundary.
- [x] Keep s.to metadata requests on verified, configured transport. Prefer reusing the existing local parser if upstream transport is not injectable; see design document.
- [x] Add regression tests for automatic mirror/IP fallback and ignored base URL.
- [x] Keep browser execution outside default request paths; test the failure path rather than assuming missing Chromium prevents it safely.
- [x] Translate missing language, absent host, blocked access, terminal source failure, and transient error distinctly.
- [x] Bound retry/request work and preserve useful diagnostics without signed URLs or cookies.
- [x] Establish upstream config-directory ownership before first import; test explicit override and non-root behavior.
- [x] Update `.env.example`, Compose forwarding, and internal configuration guidance for any new exposed setting or changed default.
...
- [x] Run API/state/STRM regressions and meaningful new contract/failure tests.
...
- [x] Repeat bounded live AniWorld and Serienstream checks with sanitized evidence.
...
- [x] Explicitly label any unavailable platform or unexercised challenge path.
...
- [x] Review only the final implementation scope; avoid pulling in catalog/WebUI/rename work.
- [x] Update relevant agent guidance if implementation changes documented behavior.
- [ ] Use the repository PR template when producing a PR body.
- [x] Explain the problem, final behavior, test evidence, and remaining provider limitations.
- [x] Do not use `Fixes #158` unless the reporter's failure condition is demonstrably fixed.
...
Implementation evidence and reasons for every open checkbox are recorded in
[08-implementation-results.md](08-implementation-results.md). In particular,
the distribution build/install portion of the combined image checkbox and the
macOS arm64 PyInstaller path passed, but their checkboxes remain open because
Linux images and the Linux/Windows binary matrix could not be exercised without
a Docker daemon or those platforms.
