...
def test_apply_release_plan_updates_version_surfaces(
    ...
    monkeypatch.setattr(cut_release, "VERSION_FILE", version_file)
    monkeypatch.setattr(cut_release, "PYPROJECT_FILE", pyproject_file)
    monkeypatch.setattr(cut_release, "OPENAPI_FILE", openapi_file)
    monkeypatch.setattr(cut_release, "DOCS_PACKAGE_FILE", package_file)
...
def test_main_outputs_release_plan_json(tmp_path: Path, monkeypatch) -> None:
    version_file = tmp_path / "VERSION"
